...
const productController = {
    ...
    detailProducts: (req, res) => {
        ...
        if (product) {
            res.render('products/productDetail', {title: "Detalles de producto", product });
        } else {
            res.status(404).send('Producto no encontrado');
        }
    },
    createForm: (req, res) => {
        res.render("products/productCreate", {title: "Formulario de edicion"});
    },
    create: (req, res) => {
        const products = getProducts();
    ...
    editForm: (req, res) => {
        ...
        if (product) {
            res.render('products/productEdit', {title: "Formulario de edicion", product });
        } else {
